  ...
  function pollCurrentSpend () {
    ...
    sns.getSMSAttributes({ attributes: [ 'MonthlySpendLimit' ] }).promise()
      ...
      .then(result => {
        ...
        log.info({ op: 'sms.budget.ok', isBudgetOk })
        log.info({ op: 'sms.budget.current', current })
        log.info({ op: 'sms.budget.limit', limit })
        log.info({ op: 'sms.budget.threshold', CREDIT_THRESHOLD })
      })
      .catch(err => {
